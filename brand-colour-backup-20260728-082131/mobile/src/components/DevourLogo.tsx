      ...
      <View style={[styles.row, { minHeight: size * 1.15 }]}>
        <View onLayout={onLayout} style={styles.word}>
          {WORD.split('').map((letter, index) => {
            ...
            return (
              <Text key={`${letter}-${index}`} style={[styles.letter, { fontSize: size, lineHeight: size * 1.05, opacity: visible ? 1 : 0 }]}>
                {letter}
              </Text>
            );
          })}
...
const styles = StyleSheet.create({
  ...
  pacman: {
    ...
    backgroundColor: colors.gold, shadowColor: '#000', shadowOpacity: 0.08,
