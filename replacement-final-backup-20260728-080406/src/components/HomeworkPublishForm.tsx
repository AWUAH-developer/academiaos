...
export function HomeworkPublishForm({ years, terms, classes, subjects }: Props) {
  ...
  const filteredTerms = useMemo(
    () =>
      academicYearId
        ? terms.filter((term) => term.academicYearId === academicYearId)
        : terms,
  ...
  return (
    <section className="paper-card p-5">
      ...
      <p className="mt-1 text-xs text-slate-500">
        Select the class and subject, enter the assignment, and attach a PDF or image when needed.
      </p>
      ...
      <form action={createHomeworkAction} className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <input
          ...
          aria-label="Due date in day month year format"
          required
        />
        ...
        <select
          className="input sm:col-span-2 xl:col-span-3"
          name="sourceType"
          value={sourceType}
