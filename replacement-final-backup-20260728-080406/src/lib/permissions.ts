...
export const navigationByRole: Record<UserRole, string[]> = {
  SUPER_ADMIN: ['dashboard','schools','demo-requests','packages','users','setup','learners','attendance','staff-attendance','fees','academics','homework','approvals','reports','transport','messages','helpdesk','audit','id-cards'],
...
export function canViewUsers(role: UserRole) {
  return ['SUPER_ADMIN','SCHOOL_ADMIN','PROPRIETOR'].includes(role);
}
