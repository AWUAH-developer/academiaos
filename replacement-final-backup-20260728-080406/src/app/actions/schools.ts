...
export async function updateUserAdmissionManagementAction(formData: FormData) {
  ...
  if (mode === 'lock') {
    ...
    revalidatePath('/schools');
    redirect(`/schools?expand=${schoolId}&success=User+and+Admission+Management+locked`);
  }
  ...
  const allowProprietorLearners =
    formData.get('allowProprietorLearners') === 'on';

  const allowProprietorStaff =
    formData.get('allowProprietorStaff') === 'on';
  ...
  redirect(
    `/schools?expand=${schoolId}&success=User+and+Admission+Management+permissions+saved`
  );
}
