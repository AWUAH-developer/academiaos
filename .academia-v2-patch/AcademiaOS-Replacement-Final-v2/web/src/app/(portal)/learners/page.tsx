...
export default async function LearnersPage({ searchParams }: { searchParams: Promise<{ q?: string; success?: string; error?: string }> }) {
  ...
  const mayAdmit = await canCreateUserAdmissionRecord(
    user.role,
    schoolId,
    'learners'
  );

  const mayManageLearnerRecords = canManageLearners(user.role);
  ...
  return <>
    ...
    <div className={`grid gap-6 ${(mayAdmit || mayPromote) ? 'xl:grid-cols-[390px_1fr]' : ''}`}>
      {(mayAdmit || mayPromote) && <section className="paper-card h-fit p-5 xl:sticky xl:top-8">
        {mayAdmit && (
            <>
<div className="flex items-center gap-3">
        ...
        <form action={createLearnerAction} className="mt-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3"><GhanaDateInput name="dateOfBirth" placeholder="DOB DD/MM/YYYY"/><select className="input" name="gender"><option value="">Gender</option><option>Male</option><option>Female</option></select></div>
        ...
        {mayPromote && (
            <div className={mayAdmit ? "mt-6 border-t border-slate-200 pt-5" : ""}>
