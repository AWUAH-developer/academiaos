...
import { assignClassTeacherAction, assignTeacherAction, createAcademicYearAction, createClassAction, createFeeCategoryAction, createFeeStructureAction, createSubjectAction, createTermAction, updateSchoolAction } from '@/app/actions/setup';
import { FlashMessage } from '@/components/FlashMessage'; import { PageHeader } from '@/components/PageHeader';
import { db } from '@/db'; import { academicYears, classes, feeCategories, feeStructures, schools, subjects, teacherAssignments, terms, users } from '@/db/schema';
...
export default async function SetupPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
 ...
 const [school, years, termRows, classRows, subjectRows, teacherRows, categories, structures, assignments] = await Promise.all([
  ...
  db.select().from(subjects).where(eq(subjects.schoolId, schoolId)).orderBy(asc(subjects.name)), db.select().from(users).where(and(eq(users.schoolId, schoolId), eq(users.status,'ACTIVE'))).orderBy(asc(users.name)),
