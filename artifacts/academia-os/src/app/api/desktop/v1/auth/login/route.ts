...
export async function POST(request: NextRequest) {
  ...
  const tempExpired           = Boolean(user && passwordValid && user.mustChangePassword && user.temporaryPasswordExpiresAt && user.temporaryPasswordExpiresAt <= now);
  ...
  await db.insert(loginAttempts).values({ schoolId: user?.schoolId, username, userId: user?.id, success: success && !noSchool, ipAddress: ip, userAgent: ua });
  ...
  if (noSchool) return desktopError(403, 'NO_SCHOOL_MEMBERSHIP', 'This account has no active school membership.');
