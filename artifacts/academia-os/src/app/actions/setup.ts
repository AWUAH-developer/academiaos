...
function allowed(role: string) { return ['SUPER_ADMIN','SCHOOL_ADMIN','HEADTEACHER','ACADEMIC_ADMIN'].includes(role); }
function dateValue(value: FormDataEntryValue | null) { const d = new Date(String(value || '')); return Number.isNaN(d.getTime()) ? null : d; }

export async function updateSchoolAction(formData: FormData) {
  const user = await requireUser();
  if (!['SUPER_ADMIN','SCHOOL_ADMIN','PROPRIETOR'].includes(user.role)) redirect('/setup?error=Permission+denied');
...
export async function createFeeCategoryAction(formData: FormData) {
  const user = await requireUser(); if (!['SUPER_ADMIN','SCHOOL_ADMIN','ACCOUNTS'].includes(user.role)) redirect('/setup?error=Permission+denied'); const schoolId = await getActiveSchoolId(user);
