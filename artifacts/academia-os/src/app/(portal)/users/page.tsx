...
export default async function UsersPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  if (!canManageUsers(actor.role)) return <div className="paper-card p-8">Access denied.</div>;
  ...
  const roles = USER_ROLES.filter((role) => role !== 'SUPER_ADMIN' && role !== 'PARENT' && role !== 'LEARNER') as UserRole[];
