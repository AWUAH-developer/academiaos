...
export default async function HomeworkPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  return <><PageHeader eyebrow="Teaching and learning" title="Homework and class assignments" description="Publish work by class and subject. Parents and learners see assignments for their linked class."/><FlashMessage success={params.success} error={params.error}/>
  {canCreate && <section className="paper-card p-5"><h2 className="flex items-center gap-2 font-black"><BookPlus size={19}/> Publish homework</h2><form action={createHomeworkAction} className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><select className="input" name="academicYearId" required>{years.map(y=><option key={y.id} value={y.id}>{y.name}</option>)}</select><select className="input" name="termId" required>{termRows.map(t=><option key={t.id} value={t.id}>{t.name}</option>)}</select><select className="input" name="classId" required><option value="">Select class</option>{classRows.map(c=><option key={c.id} value={c.id}>{c.name} {c.stream}</option>)}</select><select className="input" name="subjectId" required><option value="">Select subject</option>{subjectRows.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select><input className="input sm:col-span-2" name="title" placeholder="Assignment title" required/><input className="input" name="dueAt" type="datetime-local" required/><input className="input" name="maximumScore" type="number" min="0" step="0.01" placeholder="Maximum score"/><textarea className="input min-h-28 sm:col-span-2 xl:col-span-3" name="instructions" placeholder="Clear instructions for learners" required/><input className="input" name="attachmentUrl" type="url" placeholder="Optional attachment link"/><button className="btn-primary sm:col-span-2 xl:col-span-4">Publish homework</button></form></section>}
  <section className="paper-card mt-6 overflow-hidden"><div className="border-b border-slate-200 px-5 py-4"><h2 className="font-black">Published work</h2></div>{rows.length ? <div className="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">{rows.map(({assignment,teacherName,className,stream,subjectName,yearName,termName})=><article key={assignment.id} className="rounded-2xl border border-slate-200 bg-white p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-xs font-extrabold uppercase tracking-wider text-chalk-700">{subjectName}</p><h3 className="mt-1 font-black">{assignment.title}</h3></div><span className="status-pill bg-blue-100 text-blue-800">{assignment.status}</span></div><p className="mt-3 whitespace-pre-wrap text-sm text-slate-700">{assignment.instructions}</p><div className="mt-4 space-y-1 text-xs text-slate-500"><p><b>Class:</b> {className} {stream}</p><p><b>Due:</b> {assignment.dueAt.toLocaleString()}</p><p><b>Teacher:</b> {teacherName}</p><p>{yearName} | {termName}{assignment.maximumScore !== null ? ` | ${assignment.maximumScore} marks` : ''}</p>{assignment.attachmentUrl && <a className="font-bold text-chalk-700 underline" href={assignment.attachmentUrl} target="_blank" rel="noreferrer">Open attachment</a>}</div></article>)}</div> : <div className="p-5"><EmptyState title="No homework published" text="Assignments will appear here after a teacher publishes them."/></div>}</section></>;
}
