...
export default async function ApprovalsPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
  ...
  const reviewer = canReviewAcademics(user.role); const proprietor = canApproveAcademics(user.role);

  return <>
    <PageHeader eyebrow="Academic control" title="Approval desk" description="Academic reviewers check teacher submissions. The proprietor alone gives final approval, returns records, rejects them, or reopens locked results." />
