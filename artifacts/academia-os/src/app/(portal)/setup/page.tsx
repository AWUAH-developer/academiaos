...
export default async function SetupPage({ searchParams }: { searchParams: Promise<{ success?: string; error?: string }> }) {
 ...
 <section className="paper-card p-5"><h2 className="flex items-center gap-2 font-black"><CalendarRange size={19}/> Academic calendar</h2><div className="mt-4 grid gap-4 sm:grid-cols-2"><form action={createAcademicYearAction} className="space-y-3"><p className="text-sm font-black">New academic year</p><input className="input" name="name" placeholder="2026/2027" required/><input className="input" name="startsOn" type="date" required/><input className="input" name="endsOn" type="date" required/><label className="flex gap-2 text-sm"><input type="checkbox" name="isCurrent"/> Current year</label><button className="btn-secondary w-full">Add year</button></form><form action={createTermAction} className="space-y-3"><p className="text-sm font-black">New term</p><select className="input" name="academicYearId" required>{years.map(y=><option key={y.id} value={y.id}>{y.name}</option>)}</select><input className="input" name="name" placeholder="Term 1" required/><div className="grid grid-cols-2 gap-2"><input className="input" name="startsOn" type="date" required/><input className="input" name="endsOn" type="date" required/></div><input className="input" name="reopeningDate" type="date"/><label className="flex gap-2 text-sm"><input type="checkbox" name="isCurrent"/> Current term</label><button className="btn-secondary w-full">Add term</button></form></div><div className="mt-4 text-xs text-slate-500">{years.length} academic year(s), {termRows.length} term(s)</div></section>
