...
async function main() {
  ...
  const demoPassword =
    process.env.DEMO_PASSWORD ?? 'AcademiaDemo2026!';
  ...
  if (!targetEmail) {
    throw new Error('DEMO_TARGET_EMAIL is required.');
  }
  ...
  let school = (
    await db
      .select()
      .from(schools)
      .where(sql`lower(${schools.email}) = ${targetEmail}`)
      .limit(1)
  )[0];
  ...
  console.log('');
  console.log('===== CLINTON DEMO DATA READY =====');
  ...
  console.log('');
  console.log('Demo password for generated accounts:');
