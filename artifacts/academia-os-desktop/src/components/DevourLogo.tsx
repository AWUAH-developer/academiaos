    ...
    <span className="desktop-login-devour" role="img" aria-label="AcademiaOS">
      <span className="desktop-login-word" aria-hidden="true">
        {WORD.split('').map((letter, index) => {
          ...
          return <span className={`desktop-login-letter ${index < 8 ? 'desktop-login-letter--academia' : 'desktop-login-letter--os'}`} key={`${letter}-${index}`} style={{ opacity: visible ? 1 : 0 }}>{letter}</span>;
        })}
      </span>
