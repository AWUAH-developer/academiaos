...
const navItems = {
  ...
  users: { href: '/users', label: 'Users & staff', icon: Users }, setup: { href: '/setup', label: 'School setup', icon: Settings2 },
...
export function AppShell({ user, children }: { user: ShellUser; children: React.ReactNode }) {
  const pages = navigationByRole[user.role];
  return <div className="min-h-screen lg:grid lg:grid-cols-[286px_1fr]">
    <aside className="wood-grain hidden min-h-screen flex-col p-5 text-white lg:flex">
      ...
      <div className="mt-auto rounded-2xl border border-white/10 bg-black/10 p-4"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-amber-400 font-black text-slate-950">{user.photoUrl ? <Image src={user.photoUrl} alt={`${user.name} profile`} width={40} height={40} unoptimized className="h-10 w-10 object-cover"/> : user.name[0]?.toUpperCase()}</div><div className="min-w-0 flex-1"><p className="truncate text-sm font-black">{user.name}</p><p className="truncate text-xs text-white/55">Secure session</p></div></div><div className="mt-3 grid grid-cols-2 gap-2"><Link href="/account/change-password" className="flex items-center justify-center gap-2 rounded-xl bg-white/10 px-2 py-2.5 text-xs font-bold hover:bg-white/15"><KeyRound size={15}/> Password</Link><form action={logoutAction}><button className="flex w-full items-center justify-center gap-2 rounded-xl bg-white/10 px-2 py-2.5 text-xs font-bold hover:bg-white/15"><LogOut size={15}/> Sign out</button></form></div></div>
    </aside>
    <div className="min-w-0"><header className="wood-grain sticky top-0 z-20 border-b border-black/10 px-4 py-3 text-white lg:hidden"><div className="flex items-center justify-between gap-4"><Brand/><Link href="/account/change-password" className="grid h-10 w-10 place-items-center overflow-hidden rounded-full bg-amber-400 font-black text-slate-950">{user.photoUrl ? <Image src={user.photoUrl} alt={`${user.name} profile`} width={40} height={40} unoptimized className="h-10 w-10 object-cover"/> : user.name[0]?.toUpperCase()}</Link></div><nav className="mt-3 flex gap-2 overflow-x-auto pb-1" aria-label="Mobile navigation">{pages.map((key) => { const item = navItems[key as keyof typeof navItems]; if (!item) return null; const Icon = item.icon; return <Link key={key} href={item.href} className="flex shrink-0 items-center gap-2 rounded-xl bg-white/10 px-3 py-2 text-xs font-extrabold"><Icon size={15}/>{item.label}</Link>; })}</nav></header>
